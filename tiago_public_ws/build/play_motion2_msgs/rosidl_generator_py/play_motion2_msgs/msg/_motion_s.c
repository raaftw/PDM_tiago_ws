...
bool play_motion2_msgs__msg__motion__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("play_motion2_msgs.msg._motion.Motion", full_classname_dest, 36) == 0);
  }
  play_motion2_msgs__msg__Motion * ros_message = _ros_message;
  {  // key
  ...
  {  // times_from_start
    PyObject * field = PyObject_GetAttrString(_pymsg, "times_from_start");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->times_from_start.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'times_from_start'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * play_motion2_msgs__msg__motion__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("play_motion2_msgs.msg._motion");
  ...
  {  // times_from_start
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "times_from_start");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->times_from_start.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->times_from_start.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->times_from_start.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
